...
function name(name, reversed) {
    return `
    <span class="nameStyle ${reversed ? 'textRight marginRight1' : 'marginLeft1'}">${name}</span>
    `
}
...
function makeBody(json) {
    ...
    return `<div class="bodyStyle${isHorizontal ? 'Horizontal' : ''}">
        <div class="teamStyle${isHorizontal ? 'Horizontal' : ''}">${arr1.map(x => x).join('')}</div>
        <div class="teamStyle${isHorizontal ? 'Horizontal' : ''}">${arr2.map(x => x).join('')}</div>
    </div>
    `
}
setInterval(async () => {
